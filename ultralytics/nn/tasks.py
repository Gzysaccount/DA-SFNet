...
from ultralytics.change_model.Telu_conv import Conv_TeLU
...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        # A standalone shape-preserving module inserted into a YAML changes all
        # subsequent numeric state-dict prefixes (model.N.*). Exact-name loading
        # would then transfer only the layers before the first insertion. When a
        # plain YOLO checkpoint is loaded into an MST-LADA/DA-FFT architecture,
        # align the original layers with the non-inserted target layers and remap
        # their numeric prefixes before intersecting shapes.
        source_layers = getattr(model, "model", None)
        target_layers = getattr(self, "model", None)
        inserted_types = (MST_LADA, DA_FFT)
        source_has_inserted = source_layers is not None and any(isinstance(m, inserted_types) for m in source_layers)
        target_has_inserted = target_layers is not None and any(isinstance(m, inserted_types) for m in target_layers)
        remapped_csd = None
        if source_layers is not None and target_layers is not None and target_has_inserted and not source_has_inserted:
            target_base_indices = [i for i, m in enumerate(target_layers) if not isinstance(m, inserted_types)]
            if len(source_layers) == len(target_base_indices) and all(
                type(source_layers[src_i]) is type(target_layers[tgt_i])
                for src_i, tgt_i in enumerate(target_base_indices)
            ):
                layer_map = {src_i: tgt_i for src_i, tgt_i in enumerate(target_base_indices)}
                remapped_csd = {}
                for key, value in csd.items():
                    ...
                    if len(parts) == 3 and parts[0] == "model" and parts[1].isdigit():
                        ...
                        key = ".".join(parts)
                    remapped_csd[key] = value

        updated_csd = intersect_dicts(remapped_csd or csd, target_state)  # intersect
        ...
        if first_conv not in updated_csd and first_conv in state_dict:
            ...
            if ch == h and cw == w:
                ...
                len_updated_csd += 1
        if verbose:
            if remapped_csd is not None:
                LOGGER.info("Remapped pretrained layer indices around inserted MST-LADA/DA-FFT modules")
...
def attempt_load_one_weight(weights, device=None, inplace=True, fuse=False):
    """Load a single model weights file.
    ...
    ckpt, weight = torch_safe_load(weights)  # load ckpt
    args = {**DEFAULT_CFG_DICT, **(ckpt.get("train_args", {}))}
    model = (ckpt.get("ema") or ckpt["model"]).float()

    model.args = args
    model.pt_path = weights
    model.task = getattr(model, "task", guess_model_task(model))

    if hasattr(model, "fuse") and fuse:
        model = model.fuse()
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    """Load multiple model weights files (ensemble).

    Args:
        weights (str | Path | list): Path to weights file or list of paths.
        device (torch.device, optional): Device to load model to.
        inplace (bool): Whether to do inplace operations.
        fuse (bool): Whether to fuse model.

    Returns:
        model (torch.nn.Module): Loaded model (ensemble if multiple weights).
        ckpt (dict): Model checkpoint dictionary.
    """
    from pathlib import Path
    ...
    if len(weights) == 1:
        return attempt_load_one_weight(weights[0], device, inplace, fuse)

    models = []
    for w in weights:
        ckpt, _ = torch_safe_load(w)
        m = (ckpt.get("ema") or ckpt["model"]).float()
        m.args = {**DEFAULT_CFG_DICT, **(ckpt.get("train_args", {}))}
        m.pt_path = w
        m.task = getattr(m, "task", guess_model_task(m))
        if hasattr(m, "fuse") and fuse:
            m = m.fuse()
        m = m.eval().to(device)
        models.append(m)

    # Ensemble: average predictions
    for k in "names", "names2":
        setattr(models[0], k, getattr(models[0], k, {}))
        for m in models[1:]:
            for k, v in getattr(m, k, {}).items():
                models[0].names[k] = v
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is LSKA:  # LSKA: channels, kernel_size, reduction
                if len(args) == 0:
                    args = [c2, 7, 16]  # default: channels, kernel_size=7, reduction=16
                elif len(args) == 1:
                    args = [args[0], 7, 16]  # [channels]
                elif len(args) == 2:
                    args = [args[0], args[1], 16]  # [channels, kernel_size]
            if m is ELA:  # ELA: channels, kernel_size
                args = [c1, 7] if len(args) == 0 else [c1, args[-1] if len(args) > 1 else 7]
            if m is SCSA:  # SCSA: channels, reduction, kernel_size
                args = [c1, 8, 7] if len(args) == 0 else [c1, args[1] if len(args) > 1 else 8, args[2] if len(args) > 2 else 7]
            if m is MST_LADA:  # MST_LADA: channels, window_size, reduction, num_scales
                args = [c1, 3, 8, 3] if len(args) == 0 else [c1, *args[1:]]
            if m is DA_FFT:  # DA_FFT: channels, window_size, num_groups, direction_angles
                args = [c1, 16, 4, [0, 90]] if len(args) == 0 else [c1, *args[1:]]

            if m in {ELA, SCSA, MST_LADA, DA_FFT}:
                pass
            else:
                args = [c1, c2, *args[1:]]
